...
{
    LootGroupInvalidSelector(Loot const& loot, uint16 lootMode) : _loot(loot), _lootMode(lootMode) {}
...
bool operator()(LootStoreItem const* item) const
{
    ...
    {
        Player* player = ObjectAccessor::FindConnectedPlayer(_loot.lootOwnerGUID);
        // We use a version of the method that accepts a WorldObject* (the player is)
        if (player && !sConditionMgr->IsObjectMeetToConditions(player, item->conditions))
            return true;
    }
...
void LootTemplate::Process(Loot& loot, bool rate, uint16 lootMode, uint8 groupId) const
{
    ...
    // Preparing data for testing conditions
    Player* player = ObjectAccessor::FindConnectedPlayer(loot.lootOwnerGUID);
    ConditionSourceInfo condSourceInfo(player);

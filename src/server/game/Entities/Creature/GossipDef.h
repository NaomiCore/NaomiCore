...
struct GossipMenuItem
{
    ...
    // Gossip: gossip_menu_option new params
    uint32     CastSpellID;
    bool       IsTriggered;
    bool       AutoClose;
	//gossip_menu_option Action Data
    uint32     ActionGiveItem;
    uint32     ActionGiveItemCount;
    uint32     ActionTakeItem;
    uint32     ActionTakeItemCount;
    uint32     ActionTeleportID;
};
...
public:
    ...
    // Full method with parameters: spells, items, teleport
    uint32 AddMenuItem(int32 menuItemId, GossipOptionIcon icon, std::string const& message, uint32 sender, uint32 action, std::string const& boxMessage, uint32 boxMoney, bool coded = false, 
        uint32 castSpellId = 0, bool isTriggered = false, bool autoClose = false, 
        uint32 giveItem = 0, uint32 giveItemCount = 0, uint32 takeItem = 0, uint32 takeItemCount = 0, uint32 teleportId = 0);
    ...
    GossipMenuItemContainer& GetMenuItemsNonConst() { return _menuItems; }

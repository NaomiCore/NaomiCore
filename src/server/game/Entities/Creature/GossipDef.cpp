...
uint32 GossipMenu::AddMenuItem(int32 menuItemId, GossipOptionIcon icon, std::string const& message, uint32 sender, uint32 action, std::string const& boxMessage, uint32 boxMoney, bool coded, uint32 castSpellId, bool isTriggered, bool autoClose, uint32 giveItem, uint32 giveItemCount, uint32 takeItem, uint32 takeItemCount, uint32 teleportId)
{
    ...
    if (menuItemId == -1)
    {
        ...
        if (_menuId)
        {
            ...
            itr = std::max_element(itr, end, [](std::pair<uint32 const, GossipMenuItems> const& a, std::pair<uint32 const, GossipMenuItems> const& b)
                {
    ...
    menuItem.MenuItemIcon = uint8(icon);
    ...
    // ASSIGNING NEW PARAMETERS FROM THE BASE
    menuItem.CastSpellID = castSpellId;
    menuItem.IsTriggered = isTriggered;
    menuItem.AutoClose = autoClose;

    // Now these variables are defined in the function arguments above
    menuItem.ActionGiveItem      = giveItem;
    menuItem.ActionGiveItemCount = giveItemCount;
    menuItem.ActionTakeItem      = takeItem;
    menuItem.ActionTakeItemCount = takeItemCount;
    menuItem.ActionTeleportID    = teleportId;

    return (uint32)menuItemId;
}
...
void GossipMenu::AddMenuItem(uint32 menuId, uint32 menuItemId, uint32 sender, uint32 action)
{
    ...
    /// CALLING THE METHOD WITH NEW PARAMETERS
    AddMenuItem(itr->second.OptionID, GossipOptionIcon(itr->second.OptionIcon), strOptionText, sender, action, strBoxText, itr->second.BoxMoney, itr->second.BoxCoded, 
    itr->second.CastSpellID, itr->second.IsTriggered, itr->second.AutoClose, 
    itr->second.ActionGiveItem, itr->second.ActionGiveItemCount, 
    itr->second.ActionTakeItem, itr->second.ActionTakeItemCount, 

...
void Player::PrepareGossipMenu(WorldObject* source, uint32 menuId /*= 0*/, bool showQuests /*= false*/)
{
    ...
    // --- FORCED BASE PRIORITY (gossip_menu_id) ---
    // If we open a menu from an NPC and the default menuId (0) is passed
    if (source->GetTypeId() == TYPEID_UNIT && menuId == 0)
    {
        if (Creature* creature = source->ToCreature())
        {
            // If a custom menu is specified in the database (creature_template)
            if (uint32 templateMenuId = creature->GetCreatureTemplate()->GossipMenuId)
    ...
    // We get a list of options for our menuId
    GossipMenuItemsMapBounds menuItemBounds = sObjectMgr->GetGossipMenuItemsMapBounds(menuId);

    // If for this ID the database is empty and this is the default menu of the source, try loading menu 0
    ...
    for (GossipMenuItemsContainer::const_iterator itr = menuItemBounds.first; itr != menuItemBounds.second; ++itr)
    {
        ...
        if (Creature* creature = source->ToCreature())
        {
            // Checking NPC flags for each option
            if (itr->second.OptionNpcFlag && !(itr->second.OptionNpcFlag & npcflags))
                continue;

            switch (itr->second.OptionType)
            {
                case GOSSIP_OPTION_ARMORER: canTalk = false; break;
                case GOSSIP_OPTION_SPIRITHEALER: if (!isDead()) canTalk = false; break;
                case GOSSIP_OPTION_VENDOR:
                {
                    VendorItemData const* vendorItems = creature->GetVendorItems();
                    if (!vendorItems || vendorItems->Empty()) canTalk = false;
                    break;
                }
                ...
                case GOSSIP_OPTION_DUALSPEC_INFO:
                    if (!(GetSpecsCount() == 1 && creature->CanResetTalents(this, false) && !(GetLevel() < sWorld->getIntConfig(CONFIG_MIN_DUALSPEC_LEVEL))))
                        canTalk = false;
                    break;
                case GOSSIP_OPTION_UNLEARNTALENTS: if (!creature->CanResetTalents(this, false)) canTalk = false; break;
                case GOSSIP_OPTION_UNLEARNPETTALENTS:
                    if (!GetPet() || GetPet()->getPetType() != HUNTER_PET || GetPet()->m_spells.size() <= 1 || !creature->CanResetTalents(this, true))
                        canTalk = false;
                    break;
                case GOSSIP_OPTION_TAXIVENDOR: if (GetSession()->SendLearnNewTaxiNode(creature)) return; break;
                case GOSSIP_OPTION_BATTLEFIELD: if (!creature->isCanInteractWithBattleMaster(this, false)) canTalk = false; break;
                case GOSSIP_OPTION_STABLEPET: if (GetClass() != CLASS_HUNTER) canTalk = false; break;
                case GOSSIP_OPTION_QUESTGIVER: canTalk = false; break;
                case GOSSIP_OPTION_TRAINER:
                {
                    Trainer::Trainer const* trainer = sObjectMgr->GetTrainer(creature->GetEntry());
                    if (!trainer || !trainer->IsTrainerValidForPlayer(this)) canTalk = false;
                    break;
                }
                ...
                case GOSSIP_OPTION_AUCTIONEER: break;
                case GOSSIP_OPTION_OUTDOORPVP: if (!sOutdoorPvPMgr->CanTalkTo(this, creature, itr->second)) canTalk = false; break;
        ...
        if (canTalk)
        {
            ...
            strOptionText = optionBroadcastText ? optionBroadcastText->GetText(locale, GetGender()) : itr->second.OptionText;
            strBoxText = boxBroadcastText ? boxBroadcastText->GetText(locale, GetGender()) : itr->second.BoxText;

            // Text localization, if available
            if (locale != DEFAULT_LOCALE && !optionBroadcastText)
            {
                if (GossipMenuItemsLocale const* gossipMenuLocale = sObjectMgr->GetGossipMenuItemsLocale(menuId, itr->second.OptionID))
                {
                    // В некоторых версиях TC используется sObjectMgr->GetLocaleString
                    sObjectMgr->GetLocaleString(gossipMenuLocale->OptionText, locale, strOptionText);
                }
            }

            // Adding a menu item with ALL your new options
            menu->GetGossipMenu().AddMenuItem(
                ...
                itr->second.BoxMoney, 
                itr->second.BoxCoded,
                itr->second.CastSpellID,
                itr->second.IsTriggered,
                itr->second.AutoClose,
                itr->second.ActionGiveItem,
                itr->second.ActionGiveItemCount,
                itr->second.ActionTakeItem,
                itr->second.ActionTakeItemCount,
                itr->second.ActionTeleportID
            );
...
void Player::OnGossipSelect(WorldObject* source, uint32 gossipListId, uint32 menuId)
{
    ...
    uint32 actualSpellID = item->CastSpellID;
    bool actualTriggered = item->IsTriggered;
    bool actualAutoClose = item->AutoClose;

    // Новые параметры действий
    uint32 giveItem = item->ActionGiveItem;
    uint32 giveItemCount = item->ActionGiveItemCount;
    uint32 takeItem = item->ActionTakeItem;
    uint32 takeItemCount = item->ActionTakeItemCount;
    uint32 teleportId = item->ActionTeleportID;
    ...
    if (takeItem)
    {
        if (!HasItemCount(takeItem, takeItemCount))
        {
            SendEquipError(EQUIP_ERR_ITEM_NOT_FOUND, nullptr, nullptr, 0);
            PlayerTalkClass->SendCloseGossip();
            return;
        }
        DestroyItemCount(takeItem, takeItemCount, true);
    }

    if (giveItem)
    {
        AddItem(giveItem, giveItemCount);
    }

    if (teleportId)
    {
        if (WorldSafeLocsEntry const* loc = sWorldSafeLocsStore.LookupEntry(teleportId))
        {
            TeleportTo(loc->Continent, loc->Loc.X, loc->Loc.Y, loc->Loc.Z, GetOrientation());
        }
    }

    if (actualSpellID)
    {
        CastSpell(this, actualSpellID, actualTriggered ? TRIGGERED_FULL_MASK : TRIGGERED_NONE);
    }
    ...
    switch (gossipOptionId)
    {
        ...
        case GOSSIP_OPTION_LEARNDUALSPEC:
            ...
            {
                CastSpell(this, 63680, TRIGGERED_FULL_MASK);
                CastSpell(this, 63624, TRIGGERED_FULL_MASK);
                PrepareGossipMenu(source, menuItemData->GossipActionMenuId);
                SendPreparedGossip(source);
            }
            break;

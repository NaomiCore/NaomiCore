...
void ObjectMgr::LoadCreatureTemplates()
{
    ...
    QueryResult result = WorldDatabase.Query(
        "SELECT entry, difficulty_entry_1, difficulty_entry_2, difficulty_entry_3, KillCredit1, "      // 0-4
        "KillCredit2, modelid1, modelid2, modelid3, modelid4, "                                       // 5-9
        "name, subname, IconName, gossip_menu_id, minlevel, "                                         // 10-14
        "maxlevel, exp, faction, npcflag, speed_walk, "                                               // 15-19
        "speed_run, scale, `rank`, dmgschool, BaseAttackTime, "                                       // 20-24
        "RangeAttackTime, BaseVariance, RangeVariance, unit_class, unit_flags, "                      // 25-29
        "unit_flags2, dynamicflags, family, type, type_flags, "                                       // 30-34
        "lootid, pickpocketloot, skinloot, PetSpellDataId, VehicleId, "                               // 35-39
        "mingold, maxgold, AIName, MovementType, ctm.Ground, "                                        // 40-44
        "ctm.Swim, ctm.Flight, ctm.Rooted, ctm.Chase, ctm.Random, "                                   // 45-49
        "ctm.InteractionPauseTimer, HoverHeight, HealthModifier, ManaModifier, ArmorModifier, "       // 50-54
        "DamageModifier, ExperienceModifier, RacialLeader, movementId, RegenHealth, "                 // 55-59
        "mechanic_immune_mask, spell_school_immune_mask, flags_extra, ScriptName, StringId "          // 60-64
...
void ObjectMgr::LoadItemTemplates() {
    ...
    QueryResult result = WorldDatabase.Query(
        "SELECT entry, class, subclass, SoundOverrideSubclass, name, displayid, "     // 0-5
        "Quality, Flags, FlagsExtra, BuyCount, BuyPrice, SellPrice, InventoryType, "  // 6-12
        "AllowableClass, AllowableRace, ItemLevel, RequiredLevel, RequiredSkill, "    // 13-17
        "RequiredSkillRank, requiredspell, requiredhonorrank, RequiredCityRank, "     // 18-21
        "RequiredReputationFaction, RequiredReputationRank, maxcount, stackable, "    // 22-25
        "ContainerSlots, StatsCount, "                                                // 26-27
        // Stats 1-10
        "stat_type1, stat_value1, stat_type2, stat_value2, stat_type3, stat_value3, " // 28-33
        "stat_type4, stat_value4, stat_type5, stat_value5, stat_type6, stat_value6, " // 34-39
        "stat_type7, stat_value7, stat_type8, stat_value8, stat_type9, stat_value9, " // 40-45
        "stat_type10, stat_value10, "                                                 // 46-47
        // Stats & Resistances
        "ScalingStatDistribution, ScalingStatValue, dmg_min1, dmg_max1, dmg_type1, "  // 48-52
        "dmg_min2, dmg_max2, dmg_type2, armor, holy_res, fire_res, nature_res, "      // 53-59
        "frost_res, shadow_res, arcane_res, delay, ammo_type, RangedModRange, "       // 60-65
        // Spells 1-5
        "spellid_1, spelltrigger_1, spellcharges_1, spellppmRate_1, spellcooldown_1, spellcategory_1, spellcategorycooldown_1, " // 66-72
        "spellid_2, spelltrigger_2, spellcharges_2, spellppmRate_2, spellcooldown_2, spellcategory_2, spellcategorycooldown_2, " // 73-79
        "spellid_3, spelltrigger_3, spellcharges_3, spellppmRate_3, spellcooldown_3, spellcategory_3, spellcategorycooldown_3, " // 80-86
        "spellid_4, spelltrigger_4, spellcharges_4, spellppmRate_4, spellcooldown_4, spellcategory_4, spellcategorycooldown_4, " // 87-93
        "spellid_5, spelltrigger_5, spellcharges_5, spellppmRate_5, spellcooldown_5, spellcategory_5, spellcategorycooldown_5, " // 94-100
        // Misc & Visuals
        "bonding, description, PageText, LanguageID, PageMaterial, startquest, "      // 101-106
        "lockid, Material, sheath, RandomProperty, RandomSuffix, block, itemset, "    // 107-113
        "MaxDurability, area, Map, BagFamily, TotemCategory, "                        // 114-118
        // Sockets & Bonuses
        "socketColor_1, socketContent_1, socketColor_2, socketContent_2, "            // 119-122
        "socketColor_3, socketContent_3, socketBonus, GemProperties, "                // 123-126
        // Additional info
        "RequiredDisenchantSkill, ArmorDamageModifier, duration, ItemLimitCategory, " // 127-130
        "HolidayId, ScriptName, DisenchantID, FoodType, minMoneyLoot, maxMoneyLoot, " // 131-136
        "flagsCustom "                                                                // 137
        "FROM item_template"
    );
...
void ObjectMgr::LoadPetLevelInfo()
{
    ...
    do
    {
        ...
        if (current_level > sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL))
        {
            ...
            {
                TC_LOG_INFO("misc", "Unused (> MaxPlayerLevel in NaomiWotlk.conf) level {} in `pet_levelstats` table, ignoring.", current_level);
...
void ObjectMgr::LoadPlayerInfo()
{
    ...
    TC_LOG_INFO("server.loading", "Loading Player Create Level HP/Mana Data...");
    {
        ...
        do
        {
            ...
            if (current_level > sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL))
            {
                TC_LOG_INFO("misc", "Unused (> MaxPlayerLevel in NaomiWotlk.conf) level {} in `player_classlevelstats` table, ignoring.", current_level);
                ++count;                                    // make result loading percent "expected" correct in case disabled detail mode for example.
                continue;
            }
    ...
    TC_LOG_INFO("server.loading", "Loading Player Create Level Stats Data...");
    {
        ...
        do
        {
            ...
            if (current_level > sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL))
            {
                ...
                {
                    TC_LOG_INFO("misc", "Unused (> MaxPlayerLevel in NaomiWotlk.conf) level {} in `player_levelstats` table, ignoring.", current_level);
                    ++count;                                // make result loading percent "expected" correct in case disabled detail mode for example.
                }
                continue;
            }
    ...
    TC_LOG_INFO("server.loading", "Loading Player Create XP Data...");
    {
        ...
        do
        {
            ...
            if (current_level >= sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL))
            {
                ...
                {
                    TC_LOG_INFO("misc", "Unused (> MaxPlayerLevel in NaomiWotlk.conf) level {} in `player_xp_for_levels` table, ignoring.", current_level);
                    ++count;                                // make result loading percent "expected" correct in case disabled detail mode for example.
                }
                continue;
            }
...
void ObjectMgr::LoadGossipMenuItems()
{
    ...
    QueryResult result = WorldDatabase.Query(
    "SELECT MenuID, OptionID, OptionIcon, OptionText, OptionBroadcastTextID, "
    "OptionType, OptionNpcFlag, ActionMenuID, ActionPoiID, BoxCoded, BoxMoney, "
    "BoxText, BoxBroadcastTextID, VerifiedBuild, CastSpellID, IsTriggered, AutoClose, "
    "ActionGiveItem, ActionGiveItemCount, ActionTakeItem, ActionTakeItemCount, ActionTeleportID "
    ...
    do
    {
        ...
        // Index 13 is VerifiedBuild. We just skip it (don't read it),
        // if there is no field for it in the gMenuItem structure.

        gMenuItem.CastSpellID           = fields[14].GetUInt32();
        gMenuItem.IsTriggered           = fields[15].GetBool();
        gMenuItem.AutoClose             = fields[16].GetBool();

        // NEW PARAMETERS (indices 17-21 correspond to SELECT)
        gMenuItem.ActionGiveItem        = fields[17].GetUInt32();
        gMenuItem.ActionGiveItemCount   = fields[18].GetUInt32();
        gMenuItem.ActionTakeItem        = fields[19].GetUInt32();
        gMenuItem.ActionTakeItemCount   = fields[20].GetUInt32();
        gMenuItem.ActionTeleportID      = fields[21].GetUInt32();

        if (gMenuItem.OptionIcon >= GOSSIP_ICON_MAX)
        {
            TC_LOG_ERROR("sql.sql", "Table `gossip_menu_option` for menu {}, id {} has unknown icon id {}. Replacing with GOSSIP_ICON_CHAT", gMenuItem.MenuID, gMenuItem.OptionID, uint32(gMenuItem.OptionIcon));
    ...
    TC_LOG_INFO("server.loading", " - Loaded {} gossip_menu_option entries in {} ms", count, GetMSTimeDiffToNow(oldMSTime));
}

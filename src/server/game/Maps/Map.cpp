...
bool Map::ExistMap(uint32 mapid, int gx, int gy)
{
    ...
    if (!pf)
    {
        ...
        TC_LOG_ERROR("maps", "Please place MAP-files (*.map) in the appropriate directory ({}), or correct the DataDir setting in your NaomiWotlk.conf file.", (sWorld->GetDataPath()+"maps/"));
    }
    else
    {
        ...
        if (vmgr->isMapLoadingEnabled())
        {
            ...
            switch (result)
            {
                ...
                case VMAP::LoadResult::FileNotFound:
                    ...
                    TC_LOG_ERROR("maps", "Please place VMAP files (*.vmtree and *.vmtile) in the vmap directory ({}), or correct the DataDir setting in your NaomiWotlk.conf file.", (sWorld->GetDataPath() + "vmaps/"));
                    return false;

...
void World::LoadConfigSettings(bool reload)
{
    ...
    SetPlayerAmountLimit(sConfigMgr->GetIntDefault("PlayerLimit", 0));
    SetMotd(sConfigMgr->GetStringDefault("Motd", "Welcome to a NaomiCore Server."));
    ...
    if (reload)
    {
        ...
        if (val != m_int_configs[CONFIG_PORT_WORLD])
            TC_LOG_ERROR("server.loading", "WorldServerPort option can't be changed at NaomiWotlk.conf reload, using current value ({}).", m_int_configs[CONFIG_PORT_WORLD]);
    }
    else
    ...
    /// @todo Add MonsterSight (with meaning) in NaomiWotlk.conf or put them as define
    ...
    if (reload)
    {
        ...
        if (val != m_int_configs[CONFIG_GAME_TYPE])
            TC_LOG_ERROR("server.loading", "GameType option can't be changed at NaomiWotlk.conf reload, using current value ({}).", m_int_configs[CONFIG_GAME_TYPE]);
    }
    else
    ...
    if (reload)
    {
        ...
        if (val != m_int_configs[CONFIG_REALM_ZONE])
            TC_LOG_ERROR("server.loading", "RealmZone option can't be changed at NaomiWotlk.conf reload, using current value ({}).", m_int_configs[CONFIG_REALM_ZONE]);
    }
    else
    ...
    if (reload)
    {
        ...
        if (val != m_int_configs[CONFIG_EXPANSION])
            TC_LOG_ERROR("server.loading", "Expansion option can't be changed at NaomiWotlk.conf reload, using current value ({}).", m_int_configs[CONFIG_EXPANSION]);
    }
    else
    ...
    if (reload)
    {
        if (dataPath != m_dataPath)
            TC_LOG_ERROR("server.loading", "DataDir option can't be changed at NaomiWotlk.conf reload, using current value ({}).", m_dataPath);
    }
    else
    {
        m_dataPath = dataPath;
...
void World::Update(uint32 diff)
{
    ...
    if (m_timers[WUPDATE_PINGDB].Passed())
    {
        ...
        TC_LOG_DEBUG("sql.driver", "> Ping MySQL :: Connection Alive");

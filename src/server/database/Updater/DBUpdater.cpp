...
bool DBUpdater<T>::Update(DatabaseWorkerPool<T>& pool)
{
    ...
    {
        TC_LOG_ERROR("sql.updates", "DBUpdater: The given source directory {} does not exist, change the path to the directory where your sql directory exists (for example c:\\source\\NaomiCore). Shutting down.", sourceDirectory.generic_string());
        return false;
    }
    ...
    Path const& path)
{
    ...
    if (ret != EXIT_SUCCESS)
    {
        TC_LOG_FATAL("sql.updates", "Applying of file \'{}\' to database \'{}\' failed!" \
            ...
            "You cannot use auto-update system and import sql files from NaomiCore repository with your sql client. "
